...
import { Triangle, Square, Hammer, Layers, Circle, ArrowLeft, ArrowRight, ShieldCheck, Star, Clock } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
import { calculateDakrenovatie, type DakrenovatieInput } from '@/lib/calculator/dakrenovatie'
...
const steps = [
  ...
  { key: 'scope', title: 'Omvang', subtitle: 'Gaat het om een gedeeltelijke of volledige renovatie?' },
  { key: 'material', title: 'Dakmateriaal', subtitle: 'Welk dakmateriaal wil je gebruiken?' },
...
const scopeOptions = [
  { value: 'partial', label: 'Gedeeltelijk', description: 'Deel van het dak vernieuwen', icon: Layers },
  { value: 'full', label: 'Volledig', description: 'Volledig dak vervangen', icon: Hammer },
]

const materialOptions = [
  { value: 'tiles', label: 'Dakpannen', description: 'Keramische of betonnen dakpannen', icon: Square },
  { value: 'slate', label: 'Leien', description: 'Natuurleien of kunstleien', icon: Triangle },
  { value: 'metal', label: 'Metaal', description: 'Zinken of stalen dakbedekking', icon: Circle },
]

export function RoofRenovationCalculatorPage() {
  const [leadModalOpen, setLeadModalOpen] = useState(false)
  const [currentStep, setCurrentStep] = useState(0)
  const [input, setInput] = useState<DakrenovatieInput>({
    roofType: 'pitched',
    surfaceArea: 80,
    scope: 'full',
    material: 'tiles',
  ...
  const estimate = calculateDakrenovatie(input)
  ...
  return (
    <>
      <section className="bg-gradient-to-b from-primary-50 to-white py-16">
        <div className="mx-auto max-w-3xl px-4 text-center">
          <h1 className="text-3xl font-bold text-neutral-900 sm:text-4xl">
            Dakrenovatie kostenberekening
          </h1>
          <p className="mt-4 text-lg text-neutral-600">
            Bereken wat een dakrenovatie kost voor jouw woning.
          </p>
        </div>
      </section>
            ...
            >
              ...
              {currentStep === 0 && (
                <CalculatorStepCards
                  ...
                  onSelect={(v) => setInput({ ...input, roofType: v as DakrenovatieInput['roofType'] })}
                  columns={2}
                />
              )}
              ...
              {currentStep === 2 && (
                <CalculatorStepCards
                  ...
                  onSelect={(v) => setInput({ ...input, scope: v as DakrenovatieInput['scope'] })}
                  columns={2}
                />
              )}
              ...
              {currentStep === 3 && (
                <CalculatorStepCards
                  ...
                  onSelect={(v) => setInput({ ...input, material: v as DakrenovatieInput['material'] })}
                  columns={3}
                />
              )}
                ...
                >
                  ...
                  <div className="rounded-xl border border-accent-200 bg-accent-50 p-6">
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="flex justify-between text-sm">
                        <span className="text-accent-600">Levensduur investering</span>
                        <span className="font-medium text-accent-700">
      ...
      <LeadCaptureModal
        ...
        source="calculator_dakrenovatie"
      />
    </>
  )
}

...
import { Layers, Home, Building2, ArrowLeft, ArrowRight, ShieldCheck, Star, Clock } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
import { calculateWallInsulation, type WallInsulationInput } from '@/lib/calculator/wall-insulation'
import { LeadCaptureModal } from '@/components/leads/LeadCaptureModal'
import { CalculatorStepCards } from '@/components/calculator/CalculatorStepCards'
import { formatEUR } from '@/lib/utils/format'

const steps = [
  { key: 'wallType', title: 'Isolatiemethode', subtitle: 'Welke methode past het beste bij je woning?' },
  { key: 'surface', title: 'Oppervlakte', subtitle: 'Hoeveel m² buitenmuur wil je isoleren?' },
  { key: 'houseType', title: 'Woningtype', subtitle: 'Wat voor type woning heb je?' },
  { key: 'results', title: 'Resultaten', subtitle: 'Je geschatte kosten en besparing' },
]

const wallTypeOptions = [
  { value: 'cavity', label: 'Spouwmuurisolatie', description: 'Inblazen in bestaande spouw, meest betaalbaar', icon: Layers },
  { value: 'exterior', label: 'Buitenisolatie', description: 'Isolatieplaten aan de buitenkant, beste prestatie', icon: Home },
  { value: 'interior', label: 'Binnenisolatie', description: 'Isolatie aan de binnenkant, geen werken buiten', icon: Building2 },
]

const houseTypeOptions = [
  { value: 'row', label: 'Rijwoning', description: 'Woning in een rij, gedeelde muren', icon: Building2 },
  { value: 'semi', label: 'Halfopen bebouwing', description: 'Eén gedeelde muur', icon: Home },
  { value: 'detached', label: 'Vrijstaande woning', description: 'Volledig vrijstaand gebouw', icon: Layers },
]

export function WallInsulationCalculatorPage() {
  const [leadModalOpen, setLeadModalOpen] = useState(false)
  const [currentStep, setCurrentStep] = useState(0)
  const [input, setInput] = useState<WallInsulationInput>({
    wallType: 'cavity',
    surfaceArea: 80,
    houseType: 'semi',
  })

  const estimate = calculateWallInsulation(input)
  ...
  return (
    <>
      <section className="bg-gradient-to-b from-primary-50 to-white py-16">
        <div className="mx-auto max-w-3xl px-4 text-center">
          <h1 className="text-3xl font-bold text-neutral-900 sm:text-4xl">
            Muurisolatie kostenberekening
          </h1>
          <p className="mt-4 text-lg text-neutral-600">
            Bereken wat muurisolatie kost voor jouw woning.
          </p>
        </div>
      </section>
            ...
            >
              ...
              {currentStep === 0 && (
                <CalculatorStepCards
                  ...
                  selected={input.wallType}
                  onSelect={(v) => setInput({ ...input, wallType: v as WallInsulationInput['wallType'] })}
                  columns={3}
                />
              )}
              ...
              {currentStep === 2 && (
                <CalculatorStepCards
                  ...
                  selected={input.houseType}
                  onSelect={(v) => setInput({ ...input, houseType: v as WallInsulationInput['houseType'] })}
                  columns={3}
                />
              )}
      ...
      <LeadCaptureModal
        ...
        projectType="muurisolatie"
        source="calculator_muurisolatie"
      />
    </>
  )
}
